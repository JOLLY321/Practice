let travelPackages = [
  ...
  function addBookings(booking) {
    const newBooking = { bookingId: bookings.length + 1, ...booking };
    bookings.push(newBooking);
    return newBooking;
  }
  
  function getPackageById(id) {
    return travelPackages.find((pkg) => pkg.packageId === id);
  }
  
  function updateSlots(packageId, seatsBooked) {
    const travelPackage = getPackageById(packageId);
    if (travelPackage && travelPackage.availableSlots >= seatsBooked) {
  ...
  function getBookingByPackage(id) {
    return bookings.filter((booking) => booking.packageId === id);
  }
